...
const Projects = () => {
  ...
  const [device, setDevice] = useState<'phone' | 'tablets' | 'desktop'>("desktop")
  ...
  if(loading){
    return (
      ...
      <div className='flex items-center justify-center h-screen'>
        <Loader2Icon className='ssize-7 animate-spin text-violet-200' />
      </div>
      </>
    )
  }
    ...
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
        ...
        <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
          ...
          <div onClick={saveProject} className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
            <button disabled={isSeving} className='max-sm:hidden bg-gray-800 hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors border border-gray-700'>
              {isSeving ? <Loader2Icon className='animate-spin' size={16} /> : 
